// View 4: WMSV vs RSV Scatter
// Each point = one player-week. X = WMSV (perfect capture), Y = RSV Week (rational capture).
// Diagonal reference line shows perfect capture. Outliers (high gap) highlighted in red.

let wrInitialized = false;

function initWmsvRsv() {
  if (wrInitialized) {
    renderWmsvRsvChart();
    return;
  }
  ...
  // ── Change handlers ───────────────────────────────────────────────────────
  seasonSel.addEventListener('change', renderWmsvRsvChart);
  posSel.addEventListener('change', renderWmsvRsvChart);
  wmsvSel.addEventListener('change', renderWmsvRsvChart);
...
function renderWmsvRsvChart() {
  ...
  destroyChart('wmsv-rsv');

  const canvas = document.getElementById('chart-wmsv-rsv');
  ...
  chartInstances['wmsv-rsv'] = new Chart(ctx, {
    type: 'scatter',
    data: { datasets },
    options: {
      responsive: true,
          ...
          callbacks: {
            ...
            label: ctx => {
              ...
              return [
                ...
                `Pts:${p.points.toFixed(1)} | WMSV:${p.x.toFixed(2)} RSV:${p.y.toFixed(2)} | Start:${(p.start_prob * 100).toFixed(1)}%`
              ];
            }
          }
        }
      ...
      scales: {
        ...
        y: {
          ...
          title: {
            display: true,
            text: 'RSV Week (Rational Capture)',
            color: THEME.muted,
            font: { family: 'DM Sans', size: 12 }
          },
          ticks: { color: THEME.muted, font: { family: 'JetBrains Mono', size: 11 } },
