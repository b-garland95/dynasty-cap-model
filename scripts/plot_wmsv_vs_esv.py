"""Scatter plot: WMSV vs ESV_week with Achane 2023 W3 highlighted."""
...
OUTPUT_PATH = REPO_ROOT / "data" / "processed" / "phase1" / "wmsv_vs_esv_week.png"
...
def main() -> int:
    ...
    max_val = max(plot_df["wmsv"].max(), plot_df["esv_week"].max()) * 1.05
    ax.plot([0, max_val], [0, max_val], ls="--", lw=1, color="#999999", zorder=1, label="Perfect capture (ESV = WMSV)")
    ...
    if not achane.empty:
        ...
        ax.annotate(
            ...
            xy=(row["wmsv"], row["esv_week"]),
            xytext=(row["wmsv"] + 2.5, row["esv_week"] + 6),
            fontsize=9,
            fontweight="bold",
            color="#E24A33",
    ...
    # Pick points with large wmsv AND large capture gap (wmsv - esv_week).
    plot_df["_gap"] = plot_df["wmsv"] - plot_df["esv_week"]
    ...
    ax.set_ylabel("ESV Week (Rational Start Value — \u03c3-weighted)", fontsize=11)
    ax.set_title("WMSV vs ESV Week: How Much Value Does a Rational Owner Actually Capture?", fontsize=13, fontweight="bold")
