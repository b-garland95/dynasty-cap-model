"""Phase 2 TV scoring: calibrate ADP → ESV and score a target-season player universe.

This module is responsible for the *redraft* TV path (tv_y0 — flat initial estimate).
Multi-year dynasty trajectory application lives in src.contracts.phase3_dynasty.
"""
...
def build_phase2_tv_inputs_from_frames(
    ...
    *,
    ...
    The flat path (tv_y0 = tv_y1 = tv_y2 = tv_y3) represents the Phase 2
    redraft ADP→ESV estimate.  Apply ``apply_dynasty_tv_path()`` from
    ``src.contracts.phase3_dynasty`` to replace tv_y1–tv_y3 with a
    dynasty-calibrated trajectory.
    """
